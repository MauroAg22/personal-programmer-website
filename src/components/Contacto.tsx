const EMAIL = "mauroaglucero@gmail.com"
const LINKEDIN_URL = "https://www.linkedin.com/in/mauro-ag-lucero"
const GITHUB_URL = "https://github.com/MauroAg22"
const WHATSAPP_URL = "https://wa.me/5492657281741"
const WHATSAPP_LABEL = "+54 9 2657 28-1741"
...
const CONTACTOS = [
  { label: EMAIL, href: `mailto:${EMAIL}`, icon: EnvelopeIcon },
  { label: "LinkedIn", href: LINKEDIN_URL, icon: LinkedinIcon },
  { label: "GitHub", href: GITHUB_URL, icon: GithubIcon },
  { label: WHATSAPP_LABEL, href: WHATSAPP_URL, icon: WhatsappIcon },
]

function Contacto() {
  return (
    <section id="contacto" className="mx-auto max-w-2xl px-6 py-24 text-center">
      <h2 className="mb-4 text-3xl font-semibold text-text">Contacto</h2>
      <p className="mb-10 text-text-muted">
        ¿Charlamos? Podés encontrarme en cualquiera de estos canales.
      </p>

      <div className="flex flex-wrap items-center justify-center gap-3">
        {CONTACTOS.map(({ label, href, icon: Icon }) => (
          <a
            key={href}
            href={href}
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-2.5 text-sm font-medium text-text transition-colors hover:border-accent hover:text-accent"
          >
            <Icon />
            {label}
          </a>
